...
/**
 * 2D-Tree implementation of solving 2D range and nearest points.
 *
 */
...
public class KdTree {
    ...
    public void insert(final Point2D pointToInsert) {
        checkNotNull(pointToInsert, "Cannot insert null as point");
    ...
    private Node put(final Node node, final Point2D pointToInsert, final int level, final RectHV rect) {
        ...
        double check = node.compare(pointToInsert);

        if (check < 0 && node.left == null) {
            if (level % 2 == 0) {
        ...
        } else if (check >= 0 && node.right == null) {
            if (level % 2 == 0) {
        ...
        if (check < 0) node.left = put(node.left, pointToInsert, level + 1, rectLeft);
        else if (check > 0) node.right = put(node.right, pointToInsert, level + 1, rectRight);
    ...
    private Point2D get(final Node node, final Point2D searchedPoint, final int level) {
        ...
        double check = node.compare(searchedPoint);
        if (check < 0) return get(node.left, searchedPoint, level + 1);
        else if (check > 0) return get(node.right, searchedPoint, level + 1);
    ...
    private Point2D nearest(final Point2D queryPoint, final Node node, final Point2D currentlyClosestPoint,
        ...
        double check = node.compare(queryPoint);
        if (check < 0) {
        ...
        if (check < 0 && node.right != null) {
            nodeRectDistance = node.right.rect.distanceTo(queryPoint);
        } else if (check >= 0 && node.left != null) {
